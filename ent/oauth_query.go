...
func (oq *OAuthQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*OAuth, init func(*OAuth), assign func(*OAuth, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_auth == nil {
			continue
		}
		fk := *nodes[i].user_auth
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_auth" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
