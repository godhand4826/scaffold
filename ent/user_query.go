...
func (uq *UserQuery) QueryAuth() *OAuthQuery {
...
func (uq *UserQuery) WithAuth(opts ...func(*OAuthQuery)) *UserQuery {
	query := (&OAuthClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAuth; query != nil {
		if err := uq.loadAuth(ctx, query, nodes,
			func(n *User) { n.Edges.Auth = []*OAuth{} },
			func(n *User, e *OAuth) { n.Edges.Auth = append(n.Edges.Auth, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAuth(ctx context.Context, query *OAuthQuery, nodes []*User, init func(*User), assign func(*User, *OAuth)) error {
	...
	for _, n := range neighbors {
		fk := n.user_auth
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_auth" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_auth" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
